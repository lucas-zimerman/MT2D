...
Sprite *Load_Sprite(char *file) {
        ...
        if (fl) {
                ...
                S->Data = (char**)malloc(S->SizeY*sizeof(char*));
                Y = 0;
                while (Y < Yi) {
                        S->Data[Y] = (char*)malloc((S->SizeX + 1) * sizeof(char));
                        Y++;
                }
                Y = 0;
        ...
        //to be implemented: Sprites Scale
         ...
         if (img) {
                 if (img->type == 0) {//render as a sprite
                         out = true;
                         x = (int)pos_x + img->SizeX;
                         y = (int)pos_y + img->SizeY;
                         x1 = pos_x;
                         y1 = pos_y;
                         if (y1 < 0)y1 = 0;
                         else if (y1 > 24)y1 = y;
...
#ifdef SDL_USE
                         //lets optimize a bit...
                         if (pos_x <= 320 && pos_x + img->ScaleX >= 0) {
